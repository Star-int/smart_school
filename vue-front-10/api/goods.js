...
export default {
    ...
    getGoodsPage(page, limit, searchObj) {
        return request({
            url: `${api_name}/getGoodsFrontPage/${page}/${limit}`, // 替换为实际的接口URL地址，并添加分页参数
            method: 'get', // 请求方法，默认为GET，也可以根据实际情况设置为POST等方法
            params: searchObj
    ...
    getGoodsFrontPage(page, limit, sellerId) {
        return request({
            url: `${api_name}/getGoodsUserPage/${page}/${limit}`, // 替换为实际的接口URL地址，并添加分页参数
            method: 'get', // 请求方法，默认为GET，也可以根据实际情况设置为POST等方法
            params: { sellerId: sellerId } // 添加sellerId参数到请求参数中，以便在请求URL中使用
